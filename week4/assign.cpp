...
int main(int argc, char *argv[]) {
    ...
    M3fType grid(data, blitz::shape(nGrid, nGrid, (nGrid + 2)), blitz::neverDeleteData);
    std::complex<float>* dataComplex = reinterpret_cast<std::complex<float>*>(data);
    blitz::Array<std::complex<float>, 3> gridComplex(dataComplex, blitz::shape(nGrid, nGrid, (nGrid + 2) / 2), blitz::neverDeleteData);
    
    grid = 0.0;
    M3fType gridNPART = grid(blitz::Range::all(), blitz::Range::all(), blitz::Range(0, nGrid));
...
#pragma omp for
    ...
    assignMass(x, y, z, nGrid, 1.0/nGrid, gridNPART);
    }
}
    ...
    start = std::chrono::system_clock::now();
    fftwf_plan p;
    p = fftwf_plan_dft_r2c_3d(nGrid, nGrid, nGrid, data, reinterpret_cast<fftwf_complex*>(dataComplex), FFTW_MEASURE);
    fftwf_execute(p);
    ...
    start = std::chrono::system_clock::now();
    blitz::thirdIndex k;
    projected = blitz::max(gridNPART, k);
    end = std::chrono::system_clock::now();
    duration = end - start;
